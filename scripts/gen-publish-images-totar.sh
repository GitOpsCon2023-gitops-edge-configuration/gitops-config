...
EDGE_IMAGE_REPO=${MY_REG}
EDGE_BOM_IMAGE_TAG=${EDGE_REG}
...
for imageTag in ${list}; do
  ...
  actualEDGEImage=${actualImageRepository}/edge_bom:${imageTag}
  customEDGEImage=edge_bom-${imageTag}
  imgpkg_copy "-i" $actualEDGEImage $customEDGEImage
  ...
  flags="-i"
  if [ $comp = "EDGE-standard-packages" ] || [ $comp = "standalone-plugins-package" ] || [ $comp = "tanzu-framework-management-packages" ]; then
    flags="-b"
  fi
...
#
#    flags="-i"
#    if [ "$comp" = "EDGE-core-packages" ] || [ "$comp" = "capabilities-package" ] || [ "$comp" = "EDGE-storageclass-package" ]; then
#      flags="-b"
#    fi
#    eval $get_comp_images | while read -r image; do
#        actualImage=${actualImageRepository}/${image}
...
#    actualImage=${actualImageRepository}/EDGE-compatibility:${imageTag}
